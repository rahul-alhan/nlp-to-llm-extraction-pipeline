...
def field_accuracy(predicted: dict, truth: dict) -> dict:
    ...
    for f in KEY_FIELDS:
        t = truth.get(f)
        if t is None:
            # Field absent from ground truth — don't score it (used to count
            # silent None==None matches, which inflated accuracy).
            continue
        scored += 1
        p = predicted.get(f)
    ...
    return {"correct": correct, "total": scored, "parsed": True}
